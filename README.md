...
|---|---|---|---|---|
| [CathAction](https://huggingface.co/datasets/airvlab/CathAction) ([arXiv 2408.13126](https://arxiv.org/abs/2408.13126)) | "device" (catheter+guidewire merged — the `segmentation_human_train` masks are binary, not per-class) | ~25k segmentation masks (boxes derived via contour detection), collision boxes, action labels | CC-BY-NC-SA-4.0 | Best available device-annotated set with masks. Split into 4 independent zips — `segmentation_human_train.zip` alone is 0.14 GB and was used for the PoC below; skip the 41.8 GB action zip and 4.35 GB collision zip (wrong label space). |
